 ...
 *
 *	Demo Link: https://drive.google.com/file/d/1TdBaFvSfF9HlGu1ACKpYF40NoxxkKqW_/view?usp=sharing
 */
#include <avr/io.h>
...
enum sm4{sm4start, wait, makeSomeNoise, noNoise} state4;
void sm1tick() {
	switch(state1) {
		case sm1start:
			state1 = light1;
...
void sm3tick() {
	...
	switch(state3) {
		case sm3start:
			...
			break;
		case output:
			tempB = noise | blinkingLED | threeLEDs;
			PORTB = tempB;
			break;
		default:
			break;
